...
// 页面socket连接控制方法
  ...
  // 
  function backMsgFun(param){
      var value = $("#back"+param+"select option:selected").val();
      var _vlalue = 0;
      var msg = '';
      if(value == 0){
        msg = param+":open";
      ...
      $.ajax({
          url:ip+'management/backmessagecontr',//http://localhost:9999/
          type:'post',
          async:false,  
          data:{"message":msg},
          // dataType:'json',    //返回的数据格式：json/xml/html/script/jsonp/text
          success:function(data){
            if(data.errCode == 0){
               if(value % 2==0){   
                   $("#back"+param+"select").get(0).options[1].selected = true;     
               }else if(value % 2==1){    
                   $("#back"+param+"select").get(0).options[0].selected = true;     
               }
               layer.msg("修改成功");
  ...
  function getSatausFun(){
    ...
    $.ajax({
        url:ip+'management/backstatusFun',//http://localhost:9999/
        type:'get',
        async:false, 
        dataType:'json',    //返回的数据格式：json/xml/html/script/jsonp/text
        success:function(data){
          ...
          for(var x=0; x<len ;x++){
            ...
            var flag = data.items[x].flag;
            var count=$("#back"+name+"select").get(0).options.length;
            for(var i=0;i<count;i++){
              if($("#back"+name+"select").get(0).options[i].value == flag){
                $("#back"+name+"select").get(0).options[i].selected = true;          
                break;  
              }  
            }
          }
